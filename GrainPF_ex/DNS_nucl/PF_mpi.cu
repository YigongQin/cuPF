...
__global__ void
add_nucl(float* phi, float* alpha_m, float ini_size, int fnx, int fny){
     ...
     // printf("%d ", Nx);
      float Tinterp= ( (1.0f-delta_x)*(1.0f-delta_y)*u_3d[ offset ] + (1.0f-delta_x)*delta_y*u_3d[ offset+Nx ] \
        ...
        float rhs_psi = ((JR-JL) + (JT-JB) ) * cP.hi*cP.hi + \
                  (1.0f-ph[C]*ph[C])*( ph[C] - cP.lamd*(1.0f-ph[C]*ph[C])*( Up-1.0f) );
       ...
       // else {rand = 0.0f;}      //  ps_new[C] = ps[C] +  cP.dt * dpsi[C];
        ...
        ph_new[C] = ph[C] +  cP.dt * dpsi[C];// + rand; //cP.dt_sqrt*cP.hi*cP.eta*rnd[C+new_noi_loc];
        ...
        if ( (ph_new[C] > LS) && (ph[C] < LS) ){
           if ( (alpha<ACR) && (alpha>-ACR) ){
            ...
            float alpha1 = 0.0f; float alpha2 = 0.0f; float alpha3 = 0.0f;
              for (int locj=-NBW;locj<=NBW;locj++){
                  for (int loci=-NBW;loci<=NBW;loci++){
                     int os_loc = locj*fnx+loci+C;
                     float weight;
                     //if ((loci+locj)%2==0) {weight=0.7f;}
                     //else {weight=1.0f;}
                     float dist_C2 = loci*loci + locj*locj;
                     weight = 1.0f/dist_C2;
                     if (ph[os_loc]>LS){
